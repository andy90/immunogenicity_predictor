...
corrs <- read.table(file = here("model_train_test/acute","res_allmodels/corrs.txt"), header = TRUE)
...
ggsave(here("model_train_test/acute", "res_allmodels/Comp_spearman_acute.pdf"), width = 5, height = 5)

aucs <- read.table(file = here("model_train_test/acute", "res_allmodels/aucs0.25.txt"), header = TRUE)
...
#  coord_cartesian(ylim = c(0.5, 0.86))+
  theme(text = element_text(size=15))
ggsave(here("model_train_test/acute", "res_allmodels/Comp_auc_acute0.25.pdf"), width = 5, height = 5)
