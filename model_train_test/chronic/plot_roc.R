...
for (dom_crit in dom_nondom_crits){
  df_tpr <- read.table(file = here("model_train_test/chronic", paste("res_allmodels/tprs_full", dom_crit, ".txt", sep = "")), stringsAsFactors = FALSE)
  df_fpr <- read.table(file = here("model_train_test/chronic", paste("res_allmodels/fpr_full", dom_crit, ".txt", sep = "")), stringsAsFactors = FALSE)
  ...
  p
  ggsave(filename = here("model_train_test/chronic", paste("res_allmodels/roc_full", dom_crit, ".pdf", sep = "")), width = 6.1, height = 6)
}