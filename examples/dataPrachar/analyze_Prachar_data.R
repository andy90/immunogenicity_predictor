...
Prachar_data_file <-here("examples", "dataPrachar", "Prachar_peptides_MHC.txt")
peptides_MHC <- read.table(Prachar_data_file)
...
for (i in 1:nrow(peptides_MHC)){
  ...
  file_fasta <- here("examples", "dataPrachar", paste(pep, ".fasta", sep = ""))
  ...
  final_output_file <- here("examples", "dataPrachar", paste(pep, ".csv", sep = ""))
